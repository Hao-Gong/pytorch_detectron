...
from model.deform_conv_fpn.fpn import _FPN
...
class resnet(_FPN):
  ...
  def _init_modules(self):
    ...
    # self.RCNN_toplayer_offset = nn.Conv2d(256, 18, kernel_size=3, stride=1, padding=1)  # reduce channel
    # self.RCNN_toplayer = DeformConv2D(256, 256, kernel_size=3,  padding=1)  # reduce channel

    # Smooth layers
    self.RCNN_smooth1_offset = nn.Conv2d(256, 18, kernel_size=3, stride=1, padding=1)
    self.RCNN_smooth1 =  DeformConv2D(256, 256, kernel_size=3, padding=1)

    self.RCNN_smooth2_offset = nn.Conv2d(256, 18, kernel_size=3, stride=1, padding=1)
    self.RCNN_smooth2 =  DeformConv2D(256, 256, kernel_size=3, padding=1)

    self.RCNN_smooth3_offset = nn.Conv2d(256, 18, kernel_size=3, stride=1, padding=1)
    self.RCNN_smooth3 =  DeformConv2D(256, 256, kernel_size=3, padding=1)
    ...
    self.RCNN_top = nn.Sequential(
      nn.Conv2d(256, 512, kernel_size=cfg.POOLING_SIZE, stride=cfg.POOLING_SIZE, padding=0),
      nn.ReLU(True),
