...
from ...base_functions.utils.config import cfg
from ...base_functions.rpn.rpn_fpn import _RPN_FPN
from ...lib_c.model.roi_layers import ROIAlign, ROIPool,ROIAlignAvg
from ...base_functions.rpn.proposal_target_layer import _ProposalTargetLayer
...
class _FPN(nn.Module):
    ...
    def _init_weights(self):
        ...
        # normal_init(self.RCNN_toplayer_offset, 0, 0.01, cfg.TRAIN.TRUNCATED)
        normal_init(self.RCNN_smooth1_offset, 0, 0.01, cfg.TRAIN.TRUNCATED)
        normal_init(self.RCNN_smooth2_offset, 0, 0.01, cfg.TRAIN.TRUNCATED)
        normal_init(self.RCNN_smooth3_offset, 0, 0.01, cfg.TRAIN.TRUNCATED)
    ...
    def forward(self, im_data, im_info, gt_boxes, num_boxes):
        ...
        # Top-down
        # c5=self.RCNN_toplayer_lat(c5)
        # c5_offset=self.RCNN_toplayer_offset(c5)
        # p5 = self.RCNN_toplayer(c5,c5_offset)
        p5 = self.RCNN_toplayer(c5)
        p4 = self._upsample_add(p5, self.RCNN_latlayer1(c4))
        p4_offset=self.RCNN_smooth1_offset(p4)
        p4 = self.RCNN_smooth1(p4,p4_offset)
        p3 = self._upsample_add(p4, self.RCNN_latlayer2(c3))

        p3_offset = self.RCNN_smooth2_offset(p3)
        p3 = self.RCNN_smooth2(p3,p3_offset)
        p2 = self._upsample_add(p3, self.RCNN_latlayer3(c2))
        p2_offset = self.RCNN_smooth3_offset(p2)
        p2 = self.RCNN_smooth3(p2,p2_offset)
        ...
        if self.training and not self.class_agnostic:
            ...
            # print("select the corresponding columns according to roi labels")
