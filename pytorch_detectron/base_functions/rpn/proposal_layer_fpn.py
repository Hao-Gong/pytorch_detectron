...
from ...base_functions.rpn.generate_anchors import generate_anchors, generate_anchors_all_pyramids
from ...base_functions.rpn.bbox_transform import bbox_transform_inv, clip_boxes, clip_boxes_batch
...
class _ProposalLayer_FPN(nn.Module):
    ...
    def __init__(self, feat_stride, scales, ratios,cfg):
        ...
        self._fpn_scales = np.array(self.cfg.FPN_ANCHOR_SCALES)
        self._fpn_feature_strides = np.array(self.cfg.FPN_FEAT_STRIDES)
        self._fpn_anchor_stride = self.cfg.FPN_ANCHOR_STRIDE
    ...
    def forward(self, input):
        ...
        im_info = input[2]
        self.cfg_key = input[3]
        feat_shapes = input[4]        

        pre_nms_topN  = self.cfg[self.cfg_key].RPN_PRE_NMS_TOP_N
        post_nms_topN = self.cfg[self.cfg_key].RPN_POST_NMS_TOP_N
        nms_thresh    = self.cfg[self.cfg_key].RPN_NMS_THRESH
        min_size      = self.cfg[self.cfg_key].RPN_MIN_SIZE
