...
class _ProposalTargetLayer(nn.Module):
    ...
    def __init__(self, nclasses,cfg):
        ...
        self.cfg=cfg
        self.BBOX_NORMALIZE_MEANS = torch.FloatTensor(self.cfg.TRAIN.BBOX_NORMALIZE_MEANS)
        self.BBOX_NORMALIZE_STDS = torch.FloatTensor(self.cfg.TRAIN.BBOX_NORMALIZE_STDS)
        self.BBOX_INSIDE_WEIGHTS = torch.FloatTensor(self.cfg.TRAIN.BBOX_INSIDE_WEIGHTS)
    ...
    def forward(self, all_rois, gt_boxes, num_boxes, stage=1):
        ...
        num_images = 1
        rois_per_image = int(self.cfg.TRAIN.BATCH_SIZE / num_images)
        fg_rois_per_image = int(np.round(self.cfg.TRAIN.FG_FRACTION * rois_per_image))
    ...
    def _sample_rois_pytorch(self, all_rois, gt_boxes, fg_rois_per_image, rois_per_image, num_classes, stage=1):
        ...
        # foreground RoIs
        if stage == 1:
            fg_thresh = self.cfg.TRAIN.FG_THRESH
            bg_thresh_hi = self.cfg.TRAIN.BG_THRESH_HI
            bg_thresh_lo = self.cfg.TRAIN.BG_THRESH_LO
        elif stage == 2:
            fg_thresh = self.cfg.TRAIN.FG_THRESH_2ND
            bg_thresh_hi = self.cfg.TRAIN.FG_THRESH_2ND
            bg_thresh_lo = self.cfg.TRAIN.BG_THRESH_LO
        elif stage == 3:
            fg_thresh = self.cfg.TRAIN.FG_THRESH_3RD
            bg_thresh_hi = self.cfg.TRAIN.FG_THRESH_3RD
            bg_thresh_lo = self.cfg.TRAIN.BG_THRESH_LO
        else:
